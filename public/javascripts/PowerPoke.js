...
class PowerPoke {
  constructor() {
    this.genPokeObjCollection();
  }
  genPokeObjCollection() {
    const res = fetch(`${API_ROOT}pokemon?limit=1015&offset=0`);
    if (res.ok) {
      const collectionData = res.json();
      for (let i=0; i<collectionData.results.length; i++) {
        const pokeRes = fetch(collectionData.results[i].url);
        if (pokeRes.ok) {
          pokeCollection.push(this.buildPokeObj(pokeRes));
        }
        if (i === collectionData.result.length - 1) {
